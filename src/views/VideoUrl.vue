<template>
  <div class="create">
    <h2>What video do you want to sing to?</h2>
    <form @submit.prevent="pressed">
      <div class="name">
        <label for="name">Enter YouTube Video URL </label>
        <input type="text" id="video" v-model="url" placeholder="Video URL" />
      </div>
...
export default {
  ...
  methods: {
    ...
    //   let url = `${this.$route.name}/create-project`;
      ...
      this.secrets = await this.$axios
        ...
        .then((response) => {
          ...
          if (response.data.status == 200) {
            ...
            this.$store.dispatch('setYoutubeVideoUrl', this.url)
            console.log(this.$store.state.youtubeVideo)
            this.$router.replace({
              name: "record",
              params: { 
